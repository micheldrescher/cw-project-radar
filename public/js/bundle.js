    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            console.log(radarRootDOM); // 1) Mouse over quadrants hghlight
...
var highlightQuadrants = function highlightQuadrants() {
  d3.selectAll('.segment').on('mouseover', mouseOverQuadrant).on('mouseout', mouseOutQuadrant); // .on('click', selectQuadrant.bind({}, quadrant.order, quadrant.startAngle))
...
function mouseoverQuadrant(order) {
  ...
  d3.select('.quadrant-group-' + order).style('opacity', 1); // dim the non-selected segments

  d3.selectAll('.quadrant-group:not(.quadrant-group-' + order + ')').style('opacity', 0.3); // highlight the segment's button
  // d3.select('.button.' + order + '.full-view').style('opacity', 1)
  // dim all other buttons
  // d3.selectAll('.button.full-view:not(.' + order+')').style('opacity', 0.3)
}
